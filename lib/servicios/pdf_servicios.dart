...
class PdfService {
  static Future<void> _guardarYAbrir(Uint8List bytes, String nombre, {String ext = 'pdf'}) async {
    if (kIsWeb) {
  ...
  static pw.Widget _tabla({
    ...
    final colWidths = widths ??
        List.filled(headers.length, 1.0 / headers.length);
  ...
  static pw.Widget _footer() {
    return pw.Container(
      ...
      child: pw.Text(
        'Sabores de Bolivia — Documento generado automáticamente',
  ...
  static String _formatTS(dynamic ts) {
    ...
    } catch (_) { return 'Sin registro'; }
  }

  static Future<void> generarReporteUsuarios(String filtroEstado) async {
    final snapshot = await FirebaseFirestore.instance.collection('app-usuarios').get();
    ...
    docs.sort((a, b) {
      ...
      if (na.isEmpty) return 1; if (nb.isEmpty) return -1;
    ...
    for (final doc in docs) {
      ...
      if (data['ultimoAcceso'] != null) {
        final dias = DateTime.now().difference(data['ultimoAcceso'].toDate()).inDays;
      ...
      rows.add([
        (data['nombre'] ?? '').toString().isEmpty ? 'Sin nombre' : data['nombre'].toString(),
    ...
    pdf.addPage(pw.MultiPage(
      ...
      build: (ctx) => [
        _encabezado('Reporte de Usuarios',
            subtitulo: 'Filtro: $filtroEstado  |  Total: ${rows.length} usuario(s)'),
        pw.SizedBox(height: 16),
        _tabla(
          headers: ['Nombre', 'Correo', 'Rol', 'Registro', 'Último acceso', 'Estado'],
          rows: rows,
  ...
  static Future<void> generarExcelUsuarios(String filtroEstado) async {
    ...
    sheet.appendRow(['Nombre', 'Correo', 'Rol', 'Fecha Registro', 'Último Acceso', 'Estado']);
      ...
      build: (ctx) => pw.Column(
        ...
        children: [
          ...
          _tabla(
            ...
            rows: [
              ['Nombre',         nombre],
              ['Correo',         data['correo']?.toString() ?? data['email']?.toString() ?? 'Sin correo'],
  ...
  static Future<void> generarReporteFavoritos() async {
    ...
    for (final userDoc in snapshot.docs) {
      ...
      final favs = await FirebaseFirestore.instance
          .collection('app-usuarios').doc(userDoc.id).collection('favoritos').get();
      rows.add([
        userData['nombre']?.toString().isEmpty == true ? 'Sin nombre' : userData['nombre']?.toString() ?? 'Sin nombre',
    ...
    pdf.addPage(pw.MultiPage(
      ...
      margin: const pw.EdgeInsets.all(28),
      build: (ctx) => [
        _encabezado('Reporte de Favoritos',
  ...
  static Future<void> generarReporteFavoritosUsuario(
    ...
    pdf.addPage(pw.MultiPage(
      ...
      margin: const pw.EdgeInsets.all(32),
      build: (ctx) => [
        _encabezado('Favoritos de $nombreUsuario', subtitulo: 'Correo: $correo'),
        pw.SizedBox(height: 16),
        pw.Container(
          ...
          child: pw.Text(
            ...
            style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 11, color: _colorVerdeOsc),
          ),
        ),
        ...
        _tabla(
          ...
          widths: [0.3, 3.0],
          rows: rows.asMap().entries.map((e) => ['${e.key + 1}', e.value[0]]).toList(),
        ),
        _footer(),
      ],
    ));
  ...
  static Future<void> generarCsvFavoritos() async {
    final snapshot = await FirebaseFirestore.instance.collection('app-usuarios').get();
    final rows = <List<dynamic>>[['Usuario', 'Correo', 'Total favoritos']];
  ...
  static Future<void> generarReporteRecetas({String categoria = 'Todas', String buscar = ''}) async {
    final snapshot = await FirebaseFirestore.instance.collection('app-recetas-completas').get();
    final pdf = pw.Document();
  ...
  static Future<void> generarExcelRecetas({String categoria = 'Todas', String buscar = ''}) async {
    final snapshot = await FirebaseFirestore.instance.collection('app-recetas-completas').get();
    final excel = Excel.createExcel();
  ...
  static Future<void> generarCsvRecetas({String categoria = 'Todas', String buscar = ''}) async {
    final snapshot = await FirebaseFirestore.instance.collection('app-recetas-completas').get();
    final rows = <List<dynamic>>[['Nombre', 'Categoría', 'Calorías', 'Tiempo (min)']];
    ...
    pdf.addPage(pw.MultiPage(
      ...
      build: (ctx) => [
        ...
        pw.Text('Ingredientes',
            style: pw.TextStyle(fontSize: 13, fontWeight: pw.FontWeight.bold, color: _colorVerdeOsc)),
        pw.SizedBox(height: 8),
        if (ingredientes.isEmpty)
          pw.Text('Sin ingredientes registrados.',
              style: const pw.TextStyle(fontSize: 10, color: PdfColors.grey))
        else
          _tabla(
            headers: ['Ingrediente', 'Cantidad', 'Unidad'],
            widths: [2.5, 1.0, 1.0],
            rows: ingredientes.map((ing) => [
              ing['nombre']?.toString() ?? '',
        ...
        if (pasos.isNotEmpty) ...[
          pw.Text('Preparación',
              style: pw.TextStyle(fontSize: 13, fontWeight: pw.FontWeight.bold, color: _colorVerdeOsc)),
          pw.SizedBox(height: 8),
          ...pasos.asMap().entries.map((e) => pw.Container(
            ...
            child: pw.Row(
              ...
              children: [
                pw.Container(
                  width: 22, height: 22,
                  decoration: pw.BoxDecoration(color: _colorVerde, shape: pw.BoxShape.circle),
                  child: pw.Center(
  ...
  static Future<void> generarExcelRecetaIndividual(
    ...
    sheetInfo.appendRow(['Calorías',     '${receta['calorias'] ?? receta['calorías'] ?? 0}']);
    ...
    for (final ing in ingredientes) {
      sheetIngs.appendRow([ing['nombre'] ?? '', ing['cantidad'] ?? '', ing['unidad'] ?? '']);
    }
  ...
  static Future<void> generarCsvRecetaIndividual(
    ...
    for (final ing in ingredientes) {
      rows.add([ing['nombre'] ?? '', ing['cantidad'] ?? '', ing['unidad'] ?? '']);
    }
    rows.add([]);
  ...
  static Future<String> _nombreReceta(String? id) async {
    ...
    try {
      final doc = await FirebaseFirestore.instance
          .collection('app-recetas-completas').doc(id).get();
      if (doc.exists) return doc.data()?['nombre']?.toString() ?? 'Sin nombre';
    } catch (_) {}
    return 'Receta eliminada';
  }

  static Future<List<List<dynamic>>> _datosPlanificadores(DateTime fecha) async {
    final snapshot = await FirebaseFirestore.instance.collection('app-usuarios').get();
    final mes = fecha.month.toString().padLeft(2, '0');
    final dia = fecha.day.toString().padLeft(2, '0');
    final fechaStr = '${fecha.year}-$mes-$dia';
    final rows = <List<dynamic>>[['Usuario', 'Correo', 'Desayuno', 'Almuerzo', 'Cena']];
    ...
    for (final userDoc in docs) {
      ...
      String desayuno = 'No planificado', almuerzo = 'No planificado', cena = 'No planificado';
  ...
  static Future<void> generarReportePlanificadoresPdf(DateTime fecha) async {
    ...
    pdf.addPage(pw.MultiPage(
      ...
      margin: const pw.EdgeInsets.all(28),
      build: (ctx) => [
        _encabezado('Plan Alimenticio por Fecha',
            subtitulo: 'Fecha: $dia/$mes/${fecha.year}  |  ${rows.length - 1} usuario(s)'),
        pw.SizedBox(height: 16),
        _tabla(
    ...
    await _guardarYAbrir(await pdf.save(), 'planificadores_${fecha.year}-$mes-$dia');
  }
  ...
  static Future<void> generarExcelPlanificadores(DateTime fecha) async {
    ...
    for (final row in rows) { sheet.appendRow(row); }
    final bytes = excel.encode();
    if (bytes == null) return;
    final fechaStr = '${fecha.year}-${fecha.month.toString().padLeft(2,'0')}-${fecha.day.toString().padLeft(2,'0')}';
    await _guardarYAbrir(Uint8List.fromList(bytes), 'planificadores_$fechaStr', ext: 'xlsx');
  }
  ...
  static Future<void> generarCsvPlanificadores(DateTime fecha) async {
    ...
    await _guardarYAbrir(
      Uint8List.fromList(const ListToCsvConverter().convert(rows).codeUnits),
      'planificadores_$fechaStr', ext: 'csv',
    );
  }
