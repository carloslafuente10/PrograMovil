...
""";
    ...
    try {
      ...
      for (final doc in snapshot.docs) {
        ...
        if (ingredientes.isNotEmpty) {
          final ingTexto = ingredientes.map((ing) {
  ...
  Future<void> _consultarTantaWawa(String pregunta) async {
    ...
    final String systemPromptFinal = _systemPromptTemplate.replaceFirst(
      "{{CATALOGO}}",
      _catalogoContexto.isNotEmpty ? _catalogoContexto : "(Catálogo vacío — rechazar toda consulta de recetas)",
    );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (_historialTantaWawa.length > 20) {
          _historialTantaWawa.removeRange(0, 2); // Eliminar el turno más antiguo
        }
        ...
        setState(() {
          _respuestaTA = "El portal ancestral está inestable. Intenta de nuevo.";
          _procesando = false;
        });
      }
    } catch (e) {
      ...
      _historialTantaWawa.removeLast(); // Revertir el turno del usuario fallido
      setState(() {
        _respuestaTA = "No pude conectarme al mundo espiritual. Revisa tu conexión.";
        _procesando = false;
      });
    }
  }
  ...
  Widget _buildPanelTexto() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: _moradoNeon.withOpacity(0.1),
            blurRadius: 12,
          ),
        ],
      ),
          ...
          children: [
            ...
            if (_historialTantaWawa.length >= 2)
              Padding(
                ...
                child: Row(
                  children: [
                    Icon(Icons.memory, color: _moradoNeon.withOpacity(0.6), size: 12),
                    const SizedBox(width: 4),
                    Text(
