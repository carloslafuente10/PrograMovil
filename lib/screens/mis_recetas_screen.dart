...
class _MisRecetasScreenState extends State<MisRecetasScreen>
  ...
  static const _estadosConfig = {
    ...
    'copia':             (Color(0xFFE8F4FD), Color(0xFF0D6EFD), Icons.copy_rounded,             'Copia'),
    ...
    'rechazada_editada': (Color(0xFFFFF3CD), Color(0xFF856404), Icons.edit_note_rounded,        'Lista p/ reenviar'),
  };
      ...
      builder: (context, snap) {
        ...
        if (docs.isEmpty) {
          return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            Text(emptyMsg, style: TextStyle(color: Colors.grey[600], fontSize: 15, fontWeight: FontWeight.w600)),
            ...
            if (enRevision)
              Positioned(top: 8, left: 8,
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: const Color(0xFF0D6EFD), borderRadius: BorderRadius.circular(8)),
  ...
  void _mostrarOpciones(BuildContext context) {
    showModalBottomSheet(
      ...
      builder: (_) => _OpcionesRecetaSheet(
        docId: docId, data: data, estadosConfig: estadosConfig));
  }
}
...
class _OpcionesRecetaSheetState extends State<_OpcionesRecetaSheet> {
  ...
  bool get _puedeEditar =>
      ...
      _estado == 'rechazada' || _estado == 'rechazada_editada' || _esCopia;
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Row(children: [
          Expanded(child: _OpcionBtn(
            ...
            color: _verde, bgColor: const Color(0xFFE8F7F1),
        ...
        Row(children: [
          ...
          if (_puedeReenviar) ...[
            Expanded(child: _OpcionBtn(
              icon: Icons.send_rounded, label: 'Reenviar',
              sublabel: 'A revisión',
              color: const Color(0xFF3B82F6), bgColor: const Color(0xFFE8F4FD),
          ...
          Expanded(child: _OpcionBtn(
            ...
            color: const Color(0xFFE53935), bgColor: const Color(0xFFFFEBEE),
...
class _OpcionBtn extends StatelessWidget {
  ...
  const _OpcionBtn({required this.icon, required this.label, required this.sublabel,
...
class _DetalleRecetaPersonalSheetState extends State<_DetalleRecetaPersonalSheet> {
  ...
  Future<void> _cargar() async {
    ...
    if (mounted) setState(() { _ings = result; _cargando = false; });
  }
            ...
            child: Padding(padding: const EdgeInsets.all(20),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                if (_cargando)
                  ...
                  ..._ings.map((ing) => Padding(padding: const EdgeInsets.symmetric(vertical: 5),
                    child: Row(children: [
                      ClipRRect(borderRadius: BorderRadius.circular(8),
                        child: SizedBox(width: 36, height: 36,
                          child: (ing['foto']?.toString() ?? '').startsWith('http')
                              ? Image.network(ing['foto'], fit: BoxFit.cover, errorBuilder: (_, __, ___) => _IngPlaceholder())
                              : _IngPlaceholder())),
                      const SizedBox(width: 10),
                      Expanded(child: Text(ing['nombre']?.toString() ?? '', style: const TextStyle(fontSize: 13))),
                ...
                if (pasos.isNotEmpty) ...[
                  ...
                  const SizedBox(height: 8),
                  ...pasos.asMap().entries.map((e) => Padding(padding: const EdgeInsets.symmetric(vertical: 6),
...
class _BuscadorRecetasDBSheetState extends State<_BuscadorRecetasDBSheet> {
  ...
  Future<void> _cargarRecientes() async {
    ...
    try {
      final snap = await FirebaseFirestore.instance.collection('app-recetas-completas').limit(20).get();
  ...
  Future<void> _buscar(String q) async {
    setState(() => _buscando = true);
    try {
      QuerySnapshot snap = await FirebaseFirestore.instance.collection('app-recetas-completas')
          .where('nombre', isGreaterThanOrEqualTo: q).where('nombre', isLessThan: '${q}z').limit(20).get();
      if (snap.docs.isEmpty) {
        final qCap = q[0].toUpperCase() + q.substring(1);
        snap = await FirebaseFirestore.instance.collection('app-recetas-completas')
            .where('nombre', isGreaterThanOrEqualTo: qCap).where('nombre', isLessThan: '${qCap}z').limit(20).get();
      }
      if (mounted) setState(() { _resultados = snap.docs; _buscando = false; });
    } catch (_) { if (mounted) setState(() => _buscando = false); }
  }
    ...
    try {
      final payload = {
        ...
        'ingredientes': (receta['ingredientes'] as List? ?? []).map((i) {
          ...
          return {'ingrediente_id': m['ingrediente_id'] ?? '', 'nombre': m['nombre'] ?? '',
              'cantidad': m['cantidad'] ?? 1, 'unidad': m['unidad'] ?? '',
              'imagen': m['imagen'] ?? '', 'es_primordial': m['es_primordial'] ?? false};
        }).toList(),
        'pasos':         receta['pasos'] ?? [],
        'estado':        'copia',
        'usuarioId':     user.uid,
      ...
      final docRef = await FirebaseFirestore.instance.collection('recetas_personales').add(payload);
      if (!mounted) return;
    ...
    return DraggableScrollableSheet(
      ...
      builder: (_, scrollCtrl) => Container(
        ...
        child: Column(children: [
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Container(
                decoration: BoxDecoration(color: const Color(0xFFF5F6FA),
                    borderRadius: BorderRadius.circular(14), border: Border.all(color: Colors.grey[200]!)),
          ...
          const Divider(height: 1),
          Expanded(child: _copiando
              ? const Center(child: CircularProgressIndicator(color: _verde))
              : _resultados.isEmpty && !_buscando
                  ? Center(child: Text('No se encontraron recetas', style: TextStyle(color: Colors.grey[500])))
                  : ListView.separated(
